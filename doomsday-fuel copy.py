...
def solution(m):
    ...
    for row in range(len(m)):
        count = 0 # used to calculate terminal / non terminal
        for item in range(len(m[row])):
            if m[row][item] == 0:
                count += 1
        if count == len(m): # if the count of zeroes is equal to the length of the matrix the row is terminal
            terminal.append(row)
        else:
            non_terminal.append(row)

    probabilities = replace_probability(m)
    R, Q = get_r_and_q(probabilities, terminal_state, non_terminal_state)
...
def get_r_and_q(m, terminal_state, non_terminal_state):
    R = []
    Q = []
